...
import { getTransactionById, getTransactions } from '../services/db.js';
    ...
    <div style="display:grid;grid-template-columns:1fr 280px;gap:24px;">
      ...
      ${isSale ? `
        <div class="ledger-card" style="align-self:start;">
          ...
          ${Number(tx.transport_cost) > 0 ? ledgerRow(`Transport (${esc(tx.transport_source || '')} → ${esc(tx.transport_destination || '')})`, `₹${fmt(tx.transport_cost)}`) : ''}
          ...
          ${Number(tx.commission_amount) > 0 ? ledgerRow(`Commission (${fmt(tx.commission_percentage)}%)`, `₹${fmt(tx.commission_amount)}`) : ''}
